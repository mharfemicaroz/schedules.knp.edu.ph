...
// { id, name, department, email, rank, stats: { loadHours, overloadHours, contactHours, courseCount }, courses: [ { id, code, title, section, hours, students, schedule, room, semester, year, examDay, examSession, examRoom, examNote } ] }
...
function mapFromFacultyLoads(root) {
  ...
  return list.map((f, idx) => {
    const courses = (f.courses || []).map((c, i) => ({
      ...
      hours: toNumber(
        pick(c, ["hours", "loadHours", "creditHours", "credits"], 0)
      ),
      ...
      // Exam fields
      examDay: pick(c, ["Exam_Day", "examDay"], ""),
      examSession: pick(c, ["Exam_Session", "examSession"], ""),
      examRoom: pick(c, ["Exam_Room", "examRoom"], ""),
      examNote: pick(c, ["Exam_Note", "examNote"], ""),
    }));
    ...
    return {
      ...
      name: pick(
        f,
        ["name", "facultyName", "instructor", "lecturer"],
        "Unknown"
      ),
...
function mapFromRelational(root) {
  ...
  const courseById = new Map(
    courses.map((c) => [
      ensureId(pick(c, ["id", "courseId"]), pick(c, ["code"])),
      c,
    ])
  );
  const sectionById = new Map(
    sections.map((s) => [
      ensureId(pick(s, ["id", "sectionId"]), pick(s, ["crn"])),
      s,
    ])
  );
  ...
  assignments.forEach((a, i) => {
    const fid = ensureId(
      pick(a, ["facultyId", "instructorId", "lecturerId"]),
      undefined
    );
    ...
    const crs =
      courseById.get(ensureId(pick(sec, ["courseId"]), pick(sec, ["code"]))) ||
      {};
    if (!f) return;
    const hours = toNumber(
      pick(
        ...
        pick(
          sec,
          ["hours", "creditHours"],
          pick(crs, ["hours", "creditHours"], 0)
        )
      )
    );
    const contactHours = toNumber(
      pick(a, ["contactHours"], pick(sec, ["contactHours"], 0))
    );
    const overloadHours = Math.max(
      0,
      toNumber(pick(a, ["overload", "overloadHours"], 0))
    );
    ...
    f.courses.push({
      ...
      examDay: pick(
        sec,
        ["Exam_Day", "examDay"],
        pick(crs, ["Exam_Day", "examDay"], "")
      ),
      examSession: pick(
        sec,
        ["Exam_Session", "examSession"],
        pick(crs, ["Exam_Session", "examSession"], "")
      ),
      examRoom: pick(
        sec,
        ["Exam_Room", "examRoom"],
        pick(crs, ["Exam_Room", "examRoom"], "")
      ),
      examNote: pick(
        sec,
        ["Exam_Note", "examNote"],
        pick(crs, ["Exam_Note", "examNote"], "")
      ),
    });
...
function parseTimeToMinutes(s) {
  ...
  const inferredMeridiem = meridiemMatch
    ? meridiemMatch[1].toUpperCase()
    : undefined;
...
function expandDayRange(s) {
  ...
  const slice =
    i <= j
      ? order.slice(i, j + 1)
      : [...order.slice(i), ...order.slice(0, j + 1)];
  return slice.map(
    (x) =>
      DAY_ALIASES[x.toLowerCase()] || x.charAt(0) + x.slice(1).toLowerCase()
  );
}
...
function parseF2FDays(str) {
  ...
  const add = (v) => {
    ...
    if (!k) return;
    const norm = DAY_ALIASES[k.toLowerCase()] || k;
...
function mapFromFlatArray(arr) {
  ...
  const anyHasName =
    Array.isArray(arr) &&
    arr.some((row) => nameKeys.some((k) => row && row[k]));
  const map = new Map();
  ...
  arr.forEach((row, idx) => {
    ...
    if (anyHasName) {
      ...
      const blockCode =
        row["block code"] ?? row.blockcode ?? row.blockcode_2 ?? row.block;
      const program =
        row.programcode ?? row.program ?? row.programCode ?? row.program_code;
      const yearlevel = row.yearlevel ?? row.yearLevel;
      const fallbackLabel = [program, yearlevel, row.block]
        .filter(Boolean)
        .join(" ")
        .trim();
    ...
    if (!f.designation && (row.designation || row.title))
      f.designation = row.designation || row.title;
    if (!f.rank && (row.rank || row.title)) f.rank = row.rank || row.title;
    if (!f.employment && row.employment) f.employment = row.employment;
    if (
      (!f.loadReleaseUnits || f.loadReleaseUnits === 0) &&
      row.load_release_units != null
    )
      f.loadReleaseUnits = toNumber(row.load_release_units, 0);
    const hours = toNumber(
      row.loadHours ?? row.hours ?? row.creditHours ?? row.credits ?? row.unit,
      0
    );
    ...
    f.courses.push({
      ...
      code:
        ...
        "",
      title:
        row.title ?? row.course_title ?? row["course title"] ?? row.name ?? "",
      section:
        row.section ??
        row.sec ??
        row.blockcode_2 ??
  ...
  list.forEach((f) => {
    ...
    const release = Number.isFinite(f.loadReleaseUnits)
      ? f.loadReleaseUnits
      : toNumber(f.loadReleaseUnits, 0);
  ...
  try {
    ...
    if (Array.isArray(root)) {
      ...
      if (Array.isArray(root.facultyLoads)) {
        ...
        Array.isArray(root.faculties) ||
        Array.isArray(root.instructors)
      ) {
        ...
        const firstArrayKey = Object.keys(root).find((k) =>
    ...
    const semesters = Array.from(
      new Set(
        faculties
          .flatMap((f) => (f.courses || []).map((c) => c.semester))
          .filter(Boolean)
      )
    );
