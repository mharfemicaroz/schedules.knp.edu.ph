...
export default function BlockSchedule() {
  ...
  const [viewMode, setViewMode] = useLocalStorage(
    'blockScheduleViewMode',
    getInitialToggleState(acadData, 'blockScheduleViewMode', 'regular')
  );
      ...
      <HStack justify="space-between" mb={2} flexWrap="wrap" gap={3}>
        ...
        <HStack spacing={4}>
          ...
          {isAdmin && !isPublic && (
            <Button
              as={RouterLink}
              to={`/share/session/block/${encodeURIComponent(encodeShareBlock(block))}${
                dayFilter ? `?day=${encodeURIComponent(dayFilter)}&session=${encodeURIComponent(sessionFilter || '')}` : ''
              }`}
              leftIcon={<FiShare2 />}
              size="sm"
              colorScheme="blue"
          ...
          {!isPublic && (
            <Button as={RouterLink} to="/views/session" variant="ghost" colorScheme="brand" leftIcon={<FiArrowLeft />}>
              Back
            </Button>
          )}
        </HStack>
